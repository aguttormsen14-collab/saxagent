...
Tonen skal vÃ¦re varm, profesjonell og personlig â€“ ikke generisk selgermail.
Struktur: Takk for mÃ¸tet â†’ oppsummer det viktigste kunden sa â†’ konkret neste steg â†’ enkel call to action.
...
1. **Ã…pning** (15 sek, naturlig â€“ ikke robotaktig)
...
5. **Avslutning** â€“ hvordan booke mÃ¸te eller neste steg
...
6. **MÃ¥let for mÃ¸tet** â€“ hva er en god utgang?
...
const icons = {
  customers: "ðŸ‘¥", call: "ðŸ“ž", meeting: "ðŸ¤", linkedin: "ðŸ’¼", notes: "ðŸ“‹",
  add: "+", back: "â†", mail: "âœ‰ï¸", save: "ðŸ’¾", copy: "â§‰", check: "âœ“", trash: "ðŸ—‘",
};
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {filtered.length === 0 && (
        <Card style={{ textAlign: "center", color: C.muted, padding: "40px 24px" }}>
          {search ? "Ingen treff" : "Ingen kunder ennÃ¥ â€“ legg til din fÃ¸rste!"}
        </Card>
      )}
      ...
      <div style={{ display: "flex", gap: 4, borderBottom: `1px solid ${C.border}`, paddingBottom: 0 }}>
        {detailTabs.map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "logg" ? "ðŸ“‹ Logg" : "âœï¸ Nytt mÃ¸te"}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Btn onClick={handleGenerateSummary} disabled={!noteInput.trim() || loading === "summary"}>
              {loading === "summary" ? "â³ Genererer..." : "ðŸ“‹ Generer referat"}
            </Btn>
            <Btn onClick={handleGenerateFollowup} disabled={(!noteInput.trim() && !summary.trim()) || loading === "followup"} variant="ghost">
              {loading === "followup" ? "â³ Skriver mail..." : `${icons.mail} Generer oppfÃ¸lgingsmail`}
            </Btn>
          </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <Btn onClick={generate} disabled={!form.company.trim() || loading}>
          {loading ? "â³ Lager guide..." : "ðŸ“ž Generer samtaleguide"}
        </Btn>
      </Card>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <Btn onClick={generate} disabled={!form.company.trim() || loading}>
          {loading ? "â³ Forbereder..." : "ðŸ¤ Generer mÃ¸tebrief"}
        </Btn>
      </Card>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, height: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 8, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "4px 4px 4px 14px" }}>
        ...
        <button onClick={send} disabled={loading || !input.trim()}
          style={{ padding: "10px 16px", background: loading || !input.trim() ? C.accentDim : "linear-gradient(135deg,#0ea5e9,#0077b5)", border: "none", borderRadius: 10, color: "#fff", fontSize: 18, cursor: "pointer", alignSelf: "flex-end", marginBottom: 4 }}>â†‘</button>
      </div>
    </div>
  );
}
